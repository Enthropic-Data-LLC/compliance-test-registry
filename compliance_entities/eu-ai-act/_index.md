...
| Articles parsed (individual files) | 18 (Art. 2, 5, 6, 9, 11-14, 17-19, 27, 43, 49, 51, 53, 55, 73) in `prohibited-highrisk-gpai.md` |
...
| Open assumptions | 1 (ASSUME-EUAI-HIGHRISK-001) |
| Contested determinations | 1 (CONTEST-EUAI-HIGHRISK-001) |
...
Items 1-9 were all covered by `prohibited-highrisk-gpai.md` (batch 12). Remaining:

1. Art. 10 (Data and data governance) — PARAMETERIZED; training-data quality criteria
2. Art. 15 (Accuracy, robustness, cybersecurity) — PARAMETERIZED; overlaps ISO 27001 A.8
3. Art. 50 (Transparency for limited-risk AI) — DETERMINISTIC disclosure obligations
4. Annex III category-by-category scoping fixtures — currently handled as one classification gate

> **Currency warning.** This is the fastest-moving framework in the registry:
> obligations phase in on separate dates through 2027, and the Commission continues to
> issue implementing acts, harmonised standards requests and codes of practice. The
> `eu-ai-act` entry in `data/sources.yml` watches the consolidated text on EUR-Lex, but
> that check cannot see secondary legislation — this framework needs periodic human
> review regardless of what the currency report says.
